...
class EdgeServer {
  ...
  hasChanges() {
    const devices = Object.keys(this.deviceQueue)
    if (devices.length !== Object.keys(this.lastDeviceData).length) {
      return true
    }
    const hasChanged = devices.some((deviceId) => {
  ...
  async run() {
    ...
    logger.info(`Last device data : ${JSON.stringify(this.lastDeviceData)}`)
    logger.info(`Findings : ${JSON.stringify(findings)}`)
    const deviceData = Object.values(findings)
